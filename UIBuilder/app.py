...
def _render_assessment_pdf(assessment: dict) -> bytes:
    ...
    lines = [
        "Hurricane Hub area summary",
        "",
        f"Address: {_pdf_text(geo.get('display_name') or assessment.get('address') or 'Selected location')}",
        f"Latitude: {_format_coord(geo.get('lat') or dash.get('location', {}).get('latitude'))}",
        f"Longitude: {_format_coord(geo.get('lon') or dash.get('location', {}).get('longitude'))}",
        "",
        f"Threat score: {_pdf_text(risk.get('threat_score') or dash.get('threat', {}).get('score'))}",
        f"Threat tier: {_pdf_text(risk.get('threat_tier') or dash.get('threat', {}).get('tier'))}",
        "",
        "Top signals:",
    ]
    reasons = risk.get("threat_reasons") or dash.get("threat", {}).get("reasons") or []
    if reasons:
        lines.extend([f"  - {_pdf_text(r)}" for r in reasons[:8]])
    else:
        lines.append("  - No strong signals in this run.")

    lines.extend([
        "",
        "ZIP / local reference",
        f"Matched ZIP: {_pdf_text(assessment.get('matched_zip') or zip_info.get('zip') or zip_info.get('zip_code'))}",
        f"City: {_pdf_text(zip_info.get('city'))}",
        f"County: {_pdf_text(zip_info.get('county'))}",
        "",
        "Evacuation / infrastructure",
        f"Evacuation zone: {_pdf_text(risk.get('evacuation_level'))}",
        f"County emergency URL: {_pdf_text(zip_info.get('county_emergency_url'))}",
        f"Power outage polygons: {_pdf_text(risk.get('power_outage_polygons_in_bbox'))}",
        f"FL511 layer hits: {_pdf_text(risk.get('fl511_incident_layers_total'))}",
    ])
    ...
    for line in lines:
        if not line:
            pdf.ln(6)
            continue
        pdf.multi_cell(0, 8, txt=_fpdf_safe(line))
...
def api_report_pdf():
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name="area-summary.pdf",
    )
